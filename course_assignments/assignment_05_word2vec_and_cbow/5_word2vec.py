...
filename = maybe_download(
    os.path.join(gcl.DATA_PATH, 'text8.zip'),
    31344016)
...
with graph.as_default(), tf.device('/cpu:0'):
    ...
    # quantities that contribute to the tensor it is passed.
...
with tf.Session(graph=graph) as session:
    ...
    for step in range(num_steps):
        ...
        _, iteration_loss = session.run([optimizer, loss], feed_dict=feed_dict)
        average_loss += iteration_loss
        if step % 2000 == 0:
            if step > 0:
...
def plot(embeddings, labels):
    ...
    fig.savefig(os.path.join(gcl.OUTPUT_PATH, 'tsne_word2vec.png'))
    # pylab.show()
...
# [CBOW](http://arxiv.org/abs/1301.3781) (Continuous Bag of Words). In the CBOW
# model, instead of predicting a context word from a word vector, you predict a
# word from the sum of all the word vectors in its context. Implement and
# evaluate a CBOW model trained on the text8 dataset.
#
# ---
